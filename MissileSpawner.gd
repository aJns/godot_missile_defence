...
var rocket = preload("res://Rocket.tscn") # Will load when parsing the script.
...
func spawn_missile():
	...
	var start_v = spawn_point.direction_to(target).normalized() * (missile_speed)
	node.init(start_v, rocket_thrust, missile_speed)
	add_child(node)
	print("Missile spawn at: ", spawn_point)